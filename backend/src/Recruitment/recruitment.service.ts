...
export class RecruitmentService {
  ...
  async createjob_template(
    createjob_template: CreateJobTemplateDto,
  ): Promise<JobTemplateDocument> {
    const template = new this.jobTemplateModel(createjob_template);
    return await template.save();
  }
  async createjob_requision(
    createjob_requision: CreateJobRequisitionDto,
  ): Promise<JobRequisitionDocument> {
    const templateExists = await this.jobTemplateModel
      .findById(createjob_requision.templateId)
      .lean();
    if (!templateExists) {
      throw new NotFoundException(
        `Job template with id ${createjob_requision.templateId} not found`,
      );
    }
  ...
  async updatejob_requisition(
    requisitionId: string,
    updatejob_requisition: UpdateJobRequisitionDto,
  ): Promise<JobRequisitionDocument> {
    ...
    if (!requisition) {
      throw new NotFoundException(
        `Job requisition with requisitionId ${requisitionId} not found`,
      );
    }
  ...
  async createCVDocument(
    createCVDocumentDto: CreateCVDocumentDto,
  ...
  async createApplication(
    ...
    const requisition = await this.jobRequisitionModel
      .findOne({ requisitionId: createApplicationDto.requisitionId })
      .lean();
    if (!requisition) {
      throw new NotFoundException(
        `Job requisition with id ${createApplicationDto.requisitionId} not found`,
      );
    }
    ...
    if (existingApplication) {
      throw new Error(
        `Application already exists for candidate ${createApplicationDto.candidateId} and requisition ${createApplicationDto.requisitionId}`,
      );
    }
    ...
    const application = new this.applicationModel({
      ...
      assignedHr: createApplicationDto.assignedHr
        ? new Types.ObjectId(createApplicationDto.assignedHr)
        : undefined,
    });
  ...
  async updateApplication(
    ...
    if (!currentApplication) {
      throw new NotFoundException(
        `Application not found for candidate ${candidateId} and requisition ${requisitionId}`,
      );
    }
    ...
    const updatedApplication = await this.applicationModel.findOneAndUpdate(
      ...
      },
      {
        ...
        assignedHr: updateApplicationDto.assignedHr
          ? new Types.ObjectId(updateApplicationDto.assignedHr)
          : undefined,
      },
      { new: true },
    );
    ...
    if (!updatedApplication) {
      throw new NotFoundException(
        `Failed to update application for candidate ${candidateId} and requisition ${requisitionId}`,
      );
    }
    ...
    if (updateApplicationDto.currentStage || updateApplicationDto.status) {
      const historyRecord = new this.applicationHistoryModel({
        ...
        newStage:
          updateApplicationDto.currentStage || currentApplication.currentStage,
