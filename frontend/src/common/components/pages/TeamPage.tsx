...
export default function TeamPage({ loginPath = '/employee/login', apiPath }: TeamPageProps) {
    ...
    React.useEffect(() => {
        const fetchTeam = async () => {
            ...
            try {
                ...
                // Fetch current user (Manager) profile first
                const profileRes = await fetch(`${apiUrl}${employeeApiPath}`, {
    ...
    const CENTER_X = 50;
    const CENTER_Y = 50;
    const INITIAL_ORBIT_RADIUS = 22;
    const ORBIT_RADIUS_STEP = 15;
    const BASE_AVATAR_SIZE = 60;
    const MANAGER_AVATAR_SIZE = 120;
    ...
    const calculatePosition = (index: number, total: number, orbitIndex: number = 0) => {
        ...
        const x = CENTER_X + radius * Math.cos(angle) * 1.8;
