...
export class LeavesRequestService {
  ...
  async submitLeaveRequest(dto: CreateLeaveRequestDto): Promise<LeaveRequest> {
    ...
    if (leaveType.minTenureMonths) {
      const tenureMonths = Math.floor(
        (Date.now() - new Date(employee.dateOfHire).getTime()) / (1000 * 60 * 60 * 24 * 30)
      );
      if (tenureMonths < leaveType.minTenureMonths) {
        throw new BadRequestException('Employee does not meet minimum tenure for this leave');
      }
    }
  ...
  async attachToLeaveRequest(leaveRequestId: string, attachmentId: string): Promise<LeaveRequest> {
  ...
  async modifyPendingRequest(id: string, dto: UpdateLeaveRequestDto): Promise<LeaveRequest> {
...
async getLeaveRequestsForManager(managerId: string): Promise<LeaveRequest[]> {
  ...
  // =============================
async finalizeLeaveRequest(leaveRequestId: string, hrUserId: string, finalStatus: LeaveStatus): Promise<LeaveRequest> {
  const request = await this.leaveRequestModel.findById(leaveRequestId);
  if (!request) throw new NotFoundException('Leave request not found');

  if (request.status !== LeaveStatus.APPROVED) {
    throw new BadRequestException('Only approved leave requests can be finalized');
  }
  ...
  if (!updatedRequest) throw new NotFoundException('Leave request not found after finalization');
...
// REQ-026: HR Override
// =============================
async hrOverrideRequest(leaveRequestId: string, hrUserId: string, newStatus: LeaveStatus, reason: string): Promise<LeaveRequest> {
  const request = await this.leaveRequestModel.findById(leaveRequestId);
  if (!request) throw new NotFoundException('Leave request not found');
  ...
  if (!updatedRequest) throw new NotFoundException('Leave request not found after override');
  return updatedRequest;
}
  ...
  // =============================
async bulkProcessRequests(leaveRequestIds: string[], action: string, hrUserId: string): Promise<{ processed: number; failed: number }> {
  let processed = 0;
  let failed = 0;
  ...
  // =============================
async verifyMedicalDocuments(leaveRequestId: string, hrUserId: string, verified: boolean, notes?: string): Promise<LeaveRequest> {
  const request = await this.leaveRequestModel.findById(leaveRequestId);
  if (!request) throw new NotFoundException('Leave request not found');

  if (!request.attachmentId) {
    throw new BadRequestException('No medical documents attached to this leave request');
  }
  ...
  if (!updatedRequest) throw new NotFoundException('Leave request not found after update');
  return updatedRequest;
}
