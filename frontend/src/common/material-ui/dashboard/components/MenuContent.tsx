...
export const mainListItems: MenuItem[] = [
  ...
  },
  {
    ...
    path: '/employee/compose-notification',
    roles: ['System Admin', 'HR Admin', 'HR Manager', 'department head']
  },
  {
    text: 'Organization Changes',
    icon: <AssignmentRoundedIcon />,
    path: '/employee/manage-structure-requests',
    roles: ['System Admin', 'HR Manager']
  },
  {
    text: 'Payroll',
...
export const recruitmentSubItems: MenuItem[] = [
  {
    text: 'Overview',
    icon: <AssignmentRoundedIcon />,
    path: '/employee/recruitment_sub',
    roles: ['System Admin']
  },
  {
    text: 'Employee',
    icon: <PeopleRoundedIcon />,
    path: '/employee/recruitment_sub/employee',
    roles: ['department employee', 'HR Employee', 'HR Manager', 'System Admin']
  },
  {
    text: 'HR Employee',
    icon: <PeopleRoundedIcon />,
    path: '/employee/recruitment_sub/hr-employee',
    roles: ['HR Manager', 'HR Employee']
  },
  {
    text: 'HR Manager',
    icon: <PeopleRoundedIcon />,
    path: '/employee/recruitment_sub/hr-manager',
    roles: ['HR Manager']
  },
  {
    text: 'System Admin',
    icon: <PeopleRoundedIcon />,
    path: '/employee/recruitment_sub/system-admin',
    roles: ['System Admin']
  }
];
...
export const candidateRecruitmentSubItems: MenuItem[] = [
  {
    text: 'Overview',
    icon: <AssignmentRoundedIcon />,
    path: '/candidate/recruitment_sub'
  }
];
...
export default function MenuContent() {
  ...
  const visibleListItems = mainListItems.filter((item) => {
    ...
    if (
      item.roles &&
      item.roles.length > 0 &&
      !item.roles.some((role) => userRoles.includes(role as SystemRole))
    ) {
      return false;
    }
    return true;
  ...
  const isSelected = (text: string) => {
    ...
    if (
      ...
      return true;
    if (text === 'Employee Requests')
      return pathname === '/employee/manage-requests';
    if (text === 'Manage Employees')
      return pathname.startsWith('/employee/manage-employees');
    if (text === 'Compose Notification')
      return pathname === '/employee/compose-notification';
    if (text === 'Organization Changes')
      return pathname === '/employee/manage-structure-requests';
    if (text === 'Time Management')
      return pathname === '/employee/time-mangemeant';

    // Payroll Check
    if (text === 'Payroll' && pathname.startsWith('/employee/payroll'))
      return true;
    if (
      text === 'Tracking' &&
      pathname.startsWith('/employee/payroll/tracking')
    )
      return true;

    // Performance Checks
    if (text === 'Dashboard' && pathname === '/employee/performance/dashboard')
      return true;
    if (
      text === 'Performance Templates' &&
      pathname === '/employee/performance/templates'
    )
      return true;
    if (
      text === 'Appraisal Cycles' &&
      pathname === '/employee/performance/cycles'
    )
      return true;
    if (
      text === 'Appraisal Assignments' &&
      pathname === '/employee/performance/assignments'
    )
      return true;
    if (
      text === 'Appraisal Monitoring' &&
      pathname === '/employee/performance/monitoring'
    )
      return true;
    if (
      text === 'Manager Appraisal' &&
      pathname === '/employee/performance/manager'
    )
      return true;
    if (
      text === 'Appraisal Review Hub' &&
      pathname === '/employee/performance/manager-assignments'
    )
      return true;
    if (
      text === 'My Performance' &&
      pathname === '/employee/performance/my-records'
    )
      return true;
    if (
      text === 'Manage Disputes' &&
      pathname === '/employee/performance/manage-disputes'
    )
      return true;
    if (text === 'Disputes' && pathname === '/employee/performance/disputes')
      return true;
    if (text === 'Recruitment' && pathname.includes('/recruitment_sub'))
      return true;
    if (
      text === 'Recruitment' &&
      pathname.startsWith('/employee/recruitment_sub')
    )
      return true;

    // Leaves submenu highlight
    if (text === 'Leave Requests' && pathname === '/employee/leaves/requests')
      return true;
    if (text === 'Leave Policies' && pathname === '/employee/leaves/policy')
      return true;
    if (text === 'Entitlements' && pathname === '/employee/leaves/entitlement')
      return true;
    if (
      text === 'Leave Types' &&
      (pathname === '/employee/leaves/type' ||
        pathname === '/employee/leaves/type/special')
    )
      return true;
    if (text === 'Calendar' && pathname === '/employee/leaves/calendar')
      return true;
    if (
      text === 'Requests Dashboard' &&
      pathname === '/employee/leaves/requests/hr'
    )
      return true;
    if (
      text === 'Requests Dashboard' &&
      pathname === '/employee/leaves/requests/manager'
    )
      return true;
    if (
      text === 'Approval Flow' &&
      pathname === '/employee/leaves/requests/admin'
    )
      return true;
    if (text === 'Balance' && pathname === '/employee/leaves/balance')
      return true;
    if (text === 'History' && pathname === '/employee/leaves/history')
      return true;
    if (text === 'Automation' && pathname === '/employee/leaves/automation')
      return true;
    if (text === 'Leave Categories' && pathname === '/employee/leaves/category')
      return true;
    return false;
  };
  ...
  const handleNavigation = (text: string, path?: string) => {
    ...
    if (text === 'Organization Changes')
      router.push('/employee/manage-structure-requests');
    ...
    if (text === 'Compose Notification')
      router.push('/employee/compose-notification');
                      ...
                      if (subItem.text === 'Tracking') {
                        return (
                          <Box key={subIndex} component="div">
                            ...
                            >
                              <List component="div" disablePadding dense>
                                {trackingSubItems.map(
                                  (trackingItem, trackingIndex) => {
                                    // Role check for tracking sub-items
                                    if (!loading) {
                                      if (
                                        trackingItem.roles &&
                                        trackingItem.roles.length > 0
                                      ) {
                                        if (
                                          !trackingItem.roles.some((role) =>
                                            userRoles.includes(
                                              role as SystemRole
                                            )
                                          )
                                    ...
                                    return (
                                      <ListItemButton
                                        key={trackingIndex}
                                        sx={{ pl: 6 }}
                                        selected={isTrackingSubItemSelected(
                                          trackingItem.path || ''
                                        )}
                                        onClick={() =>
                                          trackingItem.path &&
                                          router.push(trackingItem.path)
                                        }
                                      >
                                        <ListItemIcon>
                                          {trackingItem.icon}
