...
export default function PolicyRulesSection({
  ...
  const canAssignShifts = React.useMemo(
    () =>
      normalizedRoles.some((role) =>
        ["system admin", "hr admin"].includes(role)
      ),
    [normalizedRoles]
  );

  const canDefineShiftTypes = React.useMemo(
    () =>
      normalizedRoles.some((role) =>
        ["system admin", "hr manager", "hr admin"].includes(role)
      ),
    [normalizedRoles]
  );

  const canDefineRules = React.useMemo(
    () =>
      normalizedRoles.some((role) =>
        ["hr manager", "system admin", "hr admin"].includes(role)
      ),
    [normalizedRoles]
  );

  const canSetGrace = canDefineRules;
  const canManagePolicies =
    canAssignShifts || canDefineShiftTypes || canDefineRules || canSetGrace;
  ...
  const handlePolicyToggle = React.useCallback(
    (_insightId: string, toggle: PolicyToggle) => {
      ...
      const actionKind = toggle.action.kind;
      const allowed =
        actionKind === "shift"
          ? canAssignShifts || canDefineShiftTypes
          : actionKind === "rule" || actionKind === "weekend"
          ? canDefineRules
          : canManagePolicies;
      ...
      if (!allowed) {
        setSnackbar({
          open: true,
          severity: "warning",
          message: "You are not authorized to change these policies.",
        });
        return;
      }
    ...
    [canAssignShifts, canDefineRules, canDefineShiftTypes, canManagePolicies]
  );
              ...
              {policyInsights.map((insight) => (
                <Grid size={{ xs: 12, md: 4 }} key={insight.id}>
                  <PolicyInsightCard
                    ...
                    disableInteractions={loading || !canManagePolicies}
                  />
                </Grid>
              ))}
            </Grid>
