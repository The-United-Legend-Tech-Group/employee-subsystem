...
import {
  ...
  TimeExceptionType,
  TimeExceptionStatus,
} from '../src/time-mangement/models/enums';
...
async function connect() {
  const uri =
    ...
    process.env.MONGODB_URI ||
    '';
  if (uri) return mongoose.connect(uri);

  const fakeConfig: any = {
    get: (k: string) => process.env[k] || process.env[k.toUpperCase()],
  };
  const cfg = mongooseConfigFactory(fakeConfig as any);
  return mongoose.connect(cfg.uri as string);
}
